🎬 Vibe Script ⚡

**AI-Powered Script Generator for Content Creators**

Transform raw ideas into **performance-ready scripts** using AI — with support for **Tamil, Thanglish & English**, multiple tones, and social-media-ready formats.

> ✨ Built for creators.
> 🔐 Uses your own OpenRouter API key (stored locally).
> 🚀 Deployed on Vercel.

---

Step 1: Get your api key form openrouter - https://openrouter.ai/settings/keys
<img width="973" height="945" alt="Screenshot 2026-01-01 122943" src="https://github.com/user-attachments/assets/703cefb7-1573-4048-90f5-8475bb3fc0ff" />

Step 2: Add your content and select between 3 Languages , 12 Tones and 6 Formats
<img width="1201" height="943" alt="Screenshot 2026-01-01 123013" src="https://github.com/user-attachments/assets/5a0979fd-44ec-429d-961d-34c3907d9f21" />






## 🌟 Features

✅ **3 Languages**

* English
* Tamil
* Thanglish

✅ **12 Script Tones**

* Motivational
* Funny
* Emotional
* Professional
* Casual
* Storytelling
* Bold
* Educational
* Persuasive
* Sarcastic
* Informative
* Conversational

✅ **6 Content Formats**

* Instagram Reels
* YouTube Shorts
* YouTube Videos
* Podcast Scripts
* Ad Copy
* Storytelling / Monologue

✅ **Creator-First UX**

* Paste raw ideas or rough thoughts
* Choose tone, format, language & audience
* Generate ready-to-use scripts instantly

✅ **Privacy-Friendly**

* Users bring **their own OpenRouter API key**
* API key is stored **locally in the browser**
* No backend key storage ❌

---

## 🧠 How It Works

1. User enters their **OpenRouter API key**
2. Key is stored securely in **localStorage**
3. User pastes raw content (any language)
4. Selects:

   * Language
   * Script tone
   * Format
   * Length & target audience
5. App sends request directly to **OpenRouter**
6. AI generates a polished, performance-ready script

---

## 🖥️ Live Demo

🌐 **Live App:**
👉 *Deployed on Vercel*
https://vibescript-ai-6zbe.vercel.app/
---

## 🛠️ Tech Stack

* **Frontend:** React + Vite
* **Styling:** Tailwind CSS
* **AI API:** OpenRouter
* **Deployment:** Vercel
* **State Management:** Local state + localStorage

---

## 📂 Project Structure

```
vibe-script/
│
├── public/
│   └── favicon.svg
│
├── src/
│   ├── components/
│   │   ├── ApiKeySetup.jsx
│   │   ├── ScriptInput.jsx
│   │   ├── ScriptControls.jsx
│   │   ├── ScriptOutput.jsx
│   │   └── Header.jsx
│   │
│   ├── constants/
│   │   ├── languages.js
│   │   ├── tones.js
│   │   └── formats.js
│   │
│   ├── utils/
│   │   ├── openrouter.js
│   │   └── promptBuilder.js
│   │
│   ├── App.jsx
│   ├── main.jsx
│   └── index.css
│
├── .env.example
├── package.json
├── tailwind.config.js
├── vite.config.js
└── README.md
```

---

## 🔑 OpenRouter API Setup

1. Create an account at **OpenRouter**
2. Generate an API key
3. Paste the key into **Vibe Script**
4. Click **Save API Key**
5. Start generating scripts 🎉

> 🔒 Your API key is **never sent to any server**.
> It stays **only in your browser**.

---

## 🚀 Getting Started (Local Setup)

### 1️⃣ Clone the repo

```bash
git clone https://github.com/your-username/vibe-script.git
cd vibe-script
```

### 2️⃣ Install dependencies

```bash
npm install
```

### 3️⃣ Run locally

```bash
npm run dev
```

App will run at:
👉 `http://localhost:5173`

---

## 🌍 Deployment (Vercel)

This project is **Vercel-ready**.

### Steps:

1. Fork this repository
2. Connect it to Vercel
3. Deploy 🚀

No environment variables needed (API key is user-provided).

---

## 🤝 Contributing

We welcome contributions from developers & creators 💜

### Ways to contribute:

* Add new script tones
* Add more content formats
* Improve prompt quality
* UI/UX improvements
* Bug fixes

### Contribution steps:

1. Fork the repo
2. Create a new branch
3. Make your changes
4. Open a Pull Request

---

## 📜 License

**MIT License**
Free to use, modify, and distribute.

---

## ❤️ Built For Creators

Vibe Script is made for:

* Content creators
* YouTubers
* Instagram & Reels creators
* Podcasters
* Founders & marketers
* Tamil & regional creators

---

## ⭐ Support the Project

If you like this project:

* ⭐ Star the repo
* 🐦 Share it with creators
* 🤝 Contribute

---

## ✨ Author

**Shashank P**
AI Engineer • Creator-Focused Builder

> “Turn raw thoughts into viral scripts.”

---


